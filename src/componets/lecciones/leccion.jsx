...
import {
  ...
  LeccionCuatro,
  LeccionCinco,
} from "../../data/Lecciones/index";
import { Leccion404 } from "../404Error/leccion404";
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-5">
              {leccion.palabra.map((p, idx) => {
                return (
                  <div
                    key={idx}
                    className="grid place-content-center  overflow-hidden text-center bg-gray-300/15 py-7 shadow-btn rounded-lg hover:-translate-y-2"
                  >
                    <h4 className="text-3xl font-Titulo font-bold">
                      {p.letra}
                    </h4>
                    <span className="text-gray-400 text-lg pt-2">
                      {p.traducido}
                    </span>
